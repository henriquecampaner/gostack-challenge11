...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFoods(): Promise<void> {
      try {
        ...
        const foodWithPrice = response.data.map((food: Food) => {
          return {
            ...food,
        ...
        const filtredFood = foodWithPrice.filter(
          (food: Food) => food.category === selectedCategory,
        );

        return selectedCategory
          ? setFoods(filtredFood)
          : setFoods(foodWithPrice);
      } catch (error) {
        console.log(error);
      }
    }
  ...
  function handleSelectCategory(id: number): void {
    setFoods(state => state.filter(food => food.category === id));
